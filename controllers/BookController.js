...
/* ----------------------------------- */
/**
 * Book List With Detail User.
 * 
 * @returns {Object}
 */
exports.bookListWithUser = [
    auth,
    function (req, res) {
        ...
        try {
            ...
            Book.aggregate([
                // { $match: { user: mongoose.Types.ObjectId(req.params.userid) } }
                { $match: { description: 'detail_1' } }
                ,
                { $count: "counts" }
            ]).then((counts) => {
                datas['count'] = counts[0]['counts']
                return apiResponse.successResponseWithData(res, "Operation success", datas);
            });
        } catch (err) {
            //throw error in json response with status 500. 
            return apiResponse.ErrorResponse(res, err);
